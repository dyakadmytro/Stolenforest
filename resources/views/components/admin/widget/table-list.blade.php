<div class="bg-white p-6 shadow-md">
    <table class="min-w-full">
        ...
        <tr>
            ...
            <th class="px-4 py-2 border-b border-gray-300">Posts</th>
        </tr>
        </thead>
        <tbody class="text-gray-700">
        @foreach ($widgets as $widget)
            <tr class="hover:bg-slate-200">
                <td class="px-4 py-2 border-b border-gray-300">{{$widget->name}}</td>
                <td class="px-4 py-2 border-b border-gray-300">{{$widget->posts->pluck('postable.name')->implode(', ')}}</td>
